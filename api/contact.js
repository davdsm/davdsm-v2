...
function adminEmailHtml({ name, idea, email }) {
  return `<!DOCTYPE html>
<html>
  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f4f7f5;padding:40px 16px;">
    <tr><td align="center">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:520px;background:#ffffff;border-radius:16px;overflow:hidden;">
        <tr><td style="padding:28px 32px 8px;font-size:12px;letter-spacing:0.18em;text-transform:uppercase;color:#357248;font-weight:600;">New inquiry</td></tr>
        <tr><td style="padding:0 32px 24px;font-size:28px;font-weight:700;letter-spacing:-0.03em;line-height:1.15;color:#0a1a0f;">A seed just landed.</td></tr>
        <tr><td style="padding:0 32px 28px;">
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-top:1px solid #e8eee9;">
            <tr>
              <td style="padding:18px 0 6px;font-size:11px;letter-spacing:0.16em;text-transform:uppercase;color:#7a8a7e;">Name</td>
            </tr>
            <tr><td style="padding:0 0 14px;font-size:16px;color:#0a1a0f;">${escapeHtml(name)}</td></tr>
            <tr>
              <td style="padding:8px 0 6px;font-size:11px;letter-spacing:0.16em;text-transform:uppercase;color:#7a8a7e;">Email</td>
            </tr>
            ...
            <tr>
              <td style="padding:8px 0 6px;font-size:11px;letter-spacing:0.16em;text-transform:uppercase;color:#7a8a7e;">Idea</td>
            </tr>
            <tr><td style="padding:0 0 8px;font-size:16px;line-height:1.55;color:#0a1a0f;white-space:pre-wrap;">${escapeHtml(idea)}</td></tr>
          </table>
        </td></tr>
        <tr><td style="padding:0 32px 28px;font-size:13px;color:#7a8a7e;">Reply directly to this email to reach them.</td></tr>
      </table>
...
function clientEmailHtml({ name, lang }) {
  const isPt = lang === 'pt'
  const first = escapeHtml((name || '').trim().split(/\s+/)[0] || (isPt ? '' : 'there'))
  const greeting = isPt
    ? (first ? `Obrigado, ${first}.` : 'Obrigado.')
    : `Thank you, ${first}.`
  const body1 = isPt
  ...
  const tagline = isPt ? 'Crescido, não construído.' : 'Grown, not built.'
  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f4f7f5;padding:40px 16px;">
    <tr><td align="center">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:520px;background:#ffffff;border-radius:16px;overflow:hidden;">
        ...
        <tr><td style="padding:0 32px 18px;font-size:28px;font-weight:700;letter-spacing:-0.03em;line-height:1.15;color:#0a1a0f;">${greeting}</td></tr>
        <tr><td style="padding:0 32px 24px;font-size:16px;line-height:1.65;color:#3d4a41;">${body1}</td></tr>
        <tr><td style="padding:0 32px 28px;font-size:16px;line-height:1.65;color:#3d4a41;">${body2}</td></tr>
        <tr><td style="padding:0 32px 32px;font-size:14px;color:#7a8a7e;font-style:italic;">${tagline}</td></tr>
      </table>
...
export default async function handler(req, res) {
  ...
  const firstName = name.split(/\s+/)[0] || (lang === 'pt' ? '' : 'there')
  const clientSubject =
    lang === 'pt' ? 'Obrigado — recebemos a sua mensagem' : 'Thank you — we got your message'
  const clientText =
    lang === 'pt'
  ...
  try {
    const adminResult = await resend.emails.send({
      ...
      html: adminEmailHtml({ name, idea, email }),
      text: `New inquiry\n\nName: ${name}\nEmail: ${email}\nIdea:\n${idea}\n`,
    })
    ...
    if (clientResult.error) {
      ...
      // Admin already notified — still report partial success
